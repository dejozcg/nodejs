            ...
            checks.forEach(function (check) {
                ...
                _data.read('checks', check, function (err, originalCheckData) {
                    if (!err && originalCheckData) {
                        ...
                        workers.validateChecData(originalCheckData);
                    } else {
...
workers.validateChecData = function (originalCheckData) {
    ...
    originalCheckData.method = typeof (originalCheckData.method) == 'string' && ['post', 'get', 'put', 'put'].indexOf(originalCheckData.method) > -1 ? originalCheckData.method.trim() : false;
    ...
    if (originalCheckData.id && originalCheckData.userPhone && originalCheckData.protocol && originalCheckData.url && originalCheckData.method && originalCheckData.successCodes && originalCheckData.timeoutSeconds) {
        ...
        debug("Error: one of the checks is not properly formated. Skipping.");
    }
};
...
workers.performCheck = function (originalCheckData) {
    ...
    var path = parsedUrl.path; // Using path not pathname becouse we want the query string
...
workers.alertUserToStatusChange = function(newCheckData){
    ...
    helpers.sendTwilioSms1(newCheckData.userPhone, msg, function(err){
        if(!err){
...
workers.log = function(originalCheckData, checkOutcome, state, alertWarranted, timeOfCheck){
    ...
    _logs.apend(logFileName, logString, function(err){
        if(!err){
    ...
    _logs.list(false, function(err, logs){
        if(!err && logs.length > 0){
            logs.forEach(function(logName){
                ...
                var newFileId = logName +'-' + Date.now();
...
workers.init = function () {
    ...
    // Call the commpresion loops so checks will execute later on
