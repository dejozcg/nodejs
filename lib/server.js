    ...
    req.on('end', function () {
        ...
        try{
            chosenHandler(data, function (statusCode, payloads, contentType) {
                server.processHandlerResponse(res,method,trimedPath,statusCode,payloads,contentType);
            });
        }catch(e){
            debug(e);
            server.processHandlerResponse(res,method,trimedPath,500,{'Error' : 'An undefined error is occuered'}, 'json');
        }
    });
};

// Process the response from the handler
server.processHandlerResponse = function(res,method,trimedPath,statusCode,payloads,contentType){
...
server.router = {
    ...
    'examle/error' : handlers.exampleError
};
